...
if (route(1) == "addtodo"){
    ...
    if ($post['start_date_time'] && $post['start_date']){
        $start_date = $start_date. ' ' . $post['start_date_time'];
    }
    if ($post['end_date_time'] && $post['end_date']){
        $end_date = $end_date. ' ' . $post['end_date_time'];
    }

    if ($post['category_id']){
        $user_id = get_session('id');
        $c_id = $post['category_id']; // category id
        $q = $db->query("SElECT id FROM categories WHERE user_id = '$user_id' AND categories.id = '$c_id'");
        $c = $q->fetch(PDO::FETCH_ASSOC);
        if(!$c){
            $status = 'error';
            $title = 'Ops! Dikkat';
            $msg = 'Sadece oluşturduğunuz kategorler için ekleme yapabilirsiniz.';
            echo json_encode(['status' => $status, 'title' => $title, 'msg' => $msg]);
            die;
        }
    }
...
elseif (route(1) == "removetodo"){
    ...
    $q = $db->query("DELETE FROM todos WHERE todos.user_id = '$user_id' AND todos.id = '$id'");

    if($q){
        $status = 'success';
        $title = 'İşlem Başarılı';
        $msg = 'Listeden kaldırıldı.';
